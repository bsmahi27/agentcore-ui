...
exports.buildAddAgentArgs = buildAddAgentArgs;
exports.buildAddMemoryArgs = buildAddMemoryArgs;
exports.buildAddGatewayArgs = buildAddGatewayArgs;
exports.buildAddCredentialArgs = buildAddCredentialArgs;
exports.buildAddEvaluatorArgs = buildAddEvaluatorArgs;
exports.buildDeployArgs = buildDeployArgs;
exports.buildRegistryArgs = buildRegistryArgs;
exports.buildInvokeArgs = buildInvokeArgs;
exports.buildLogsArgs = buildLogsArgs;
exports.buildStatusArgs = buildStatusArgs;
exports.buildTracesListArgs = buildTracesListArgs;
exports.buildRemoveArgs = buildRemoveArgs;
exports.argsToCommand = argsToCommand;
/** Push --flag value only when value is a non-empty string */
function kv(args, flag, value) {
    if (value && value.trim())
        args.push(flag, value.trim());
}
/** Push bare flag only when condition is truthy */
function flag(args, f, condition) {
    if (condition)
        args.push(f);
}
// ---------------------------------------------------------------------------
...
// agentcore add agent
// ---------------------------------------------------------------------------
function buildAddAgentArgs(d) {
    const args = ['add', 'agent'];
    kv(args, '--name', d.name);
    kv(args, '--type', d.type);
    if (d.type === 'byo') {
...
// ---------------------------------------------------------------------------
function buildAddMemoryArgs(d) {
    const args = ['add', 'memory'];
    kv(args, '--name', d.name);
...
// ---------------------------------------------------------------------------
function buildAddGatewayArgs(d) {
    const args = ['add', 'gateway'];
    kv(args, '--name', d.name);
...
// ---------------------------------------------------------------------------
function buildAddCredentialArgs(d) {
    const args = ['add', 'credential'];
    kv(args, '--name', d.name);
    if (d.type === 'oauth') {
...
// ---------------------------------------------------------------------------
function buildAddEvaluatorArgs(d) {
    const args = ['add', 'evaluator'];
    kv(args, '--name', d.name);
...
// ---------------------------------------------------------------------------
function buildRegistryArgs(d) {
    const args = ['bedrock-agentcore-control', 'create-registry-record'];
...
// ---------------------------------------------------------------------------
function buildTracesListArgs(d) {
    const args = ['traces', 'list'];
    kv(args, '--runtime', d.runtime);
    kv(args, '--limit', d.limit);
    kv(args, '--since', d.since);
    kv(args, '--until', d.until);
...
// agentcore remove
// ---------------------------------------------------------------------------
function buildRemoveArgs(d) {
    const args = ['remove', d.resourceType];
    if (d.resourceType !== 'all')
        kv(args, '--name', d.name);
    flag(args, '-y', d.autoConfirm);
    flag(args, '--dry-run', d.dryRun);
    args.push('--json');
...
// ---------------------------------------------------------------------------
function argsToCommand(args, executable = 'agentcore') {
